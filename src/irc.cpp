...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #KrymCoinTEST2\r");
            Send(hSocket, "WHO #KrymCoinTEST2\r");
        } else {
            // randomly join #KrymCoin00-#KrymCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #KrymCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #KrymCoin%02d\r", channel_number).c_str());
        }
