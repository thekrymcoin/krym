...
const std::string CLIENT_NAME("KrymCoin");
...
#define BUILD_DESC_FROM_COMMIT(maj,min,rev,commit) \
    "KRYM-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-g" commit

#define BUILD_DESC_FROM_UNKNOWN(maj,min,rev) \
    "KRYM-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-unk"
