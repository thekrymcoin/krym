...
unsigned int nStakeMinAge = 30 * 24 * nOneHour; // hours as zero time weight
unsigned int nStakeMaxAge = -1; //  days as full weight
unsigned int nStakeTargetSpacing = 5 * 60; // 5-minute stakes spacing
int64_t nTargetSpacing = 5 * 60;  // Same as the above
unsigned int nModifierInterval = 0.15 * nOneHour; // time to elapse before new modifier is computed
...
const string strMessageMagic = "KrymCoin Signed Message:\n";
...
{
    int64_t nSubsidy = 0.2686 * COIN;
    
	if (nHeight <= 1000)
		return 268600 * COIN;
	
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nBits=0x%08x", FormatMoney(nSubsidy).c_str(), nBits, nSubsidy);
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("KrymCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in KrymCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // KrymCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "KrymCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Start KrymCoin! HI!";
        CTransaction txNew;
        txNew.nTime = 1508752800;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1508752800;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1119917 : 1119917;
...
//        hashGenesisBlock = block.GetHash();
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x473a98c7a7f858a6f12ef75212d69299f12b67d96bfc989128a2452af4c3640a"));
